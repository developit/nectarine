...
export default ({ url = URL, store, imgurKey, init = true } = {}) => {
	...
	peach.user.stream = (id, callback = EMPTY_FUNC, opts = {}) => {
		...
		if (!opts.optimistic && cached && peach.streamCache.hasOwnProperty(id)) {
			if (
				cached &&
				cached._fetched &&
				Date.now() - cached._fetched < CACHE_LIFETIME
			) {
	...
	let hasComments = (stream) =>
		stream &&
		stream.posts &&
		stream.posts.filter((p) => p.hasOwnProperty("comments")).length > 0;
	...
	peach.post = (post, callback) => {
		...
		api.post(
			{ url: "/post", body: { message: [].concat(post) } },
			cb(callback),
		);
	};

	/** Delete a post. */
	peach.deletePost = (id, callback) =>
		api.delete(`/post/${enc(id)}`, cb(callback));
	...
	peach.wave = (targetStreamId, type, callback) => {
		api.post(
			{ url: "/activity/wave", body: { targetStreamId, type } },
			cb(callback),
		);
	};

	/** Like a post */
	peach.like = (postId, callback) =>
		api.post({ url: "/like", body: { postId } }, cb(callback));

	/** Un-like a post */
	peach.unlike = (postId, callback) =>
		api.delete(`/like/postID/${postId}`, cb(callback));
	 ...
	 */
	peach.deleteComment = (id, callback) =>
		api.delete(`/comment/${enc(id)}`, cb(callback));
	...
	peach.setAvatar = (avatarSrc, callback = EMPTY_FUNC) => {
		api.put(
			{ url: "/stream/avatarSrc", body: { avatarSrc } },
			(err, res, data = {}) => {
	...
	peach.addFriend = (username, callback) =>
		api.post(`/stream/n/${enc(username)}/connection`, cb(callback));
	...
	/** Issue a friend request */
	peach.acceptFriendRequest = (id, callback) =>
		api.post(`/friend-request/${enc(id)}/accept`, cb(callback));

	/** Deny a friend request */
	peach.denyFriendRequest = (id, callback) =>
		api.delete(`/friend-request/${enc(id)}`, cb(callback));
