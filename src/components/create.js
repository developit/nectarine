...
export default class Create extends Component {
	...
	@bind
	submit() {
		let { text, type, gifs, images, uploading, loading, selectedMagic } =
			this.state;
		if (loading || uploading) return alert("Still uploading...");
		if (!text && type === "text")
			return this.setState({ error: "Enter a message" });
	...
	magicGif(terms) {
		...
		fetch(
			`https://api.giphy.com/v1/gifs/search?q=${encodeURIComponent(terms)}&api_key=dc6zaTOxFJmzC`,
		)
			.then((r) => r.json())
			.then(({ data = [] }) => {
				this.setState({
	...
	addImages() {
		chooseFiles((files) => {
			...
			parallel(
				files.map((img) => [this.uploadImage, img]),
				(error, ...images) => {
	...
	render(
		{},
		{ open, text = "", error, uploading, loading, images, gifs, selectedMagic },
	) {
		return (
			<div class="create modal" showing={open || null}>
				<Layout.Header manual>
					<Layout.HeaderRow>
						<Button icon onClick={this.close}>
							<Icon icon="close" />
						</Button>
				...
				<div class="content has-header" magic={!!gifs || null}>
					<div class="inner">
						...
						<div class="images" show={(images && images.length) || null}>
							<div class="inner">
								{images ? images.map((img) => <img src={img.url} />) : null}
							</div>
						</div>
					</div>
					{error ? (
